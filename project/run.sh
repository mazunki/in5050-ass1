...
pipeline() {
	...
	runner "cd '${WORKDIR}' && nsys profile -o '${REPORT_FILE_ENC}' -- ${cmd_enc}" || { echo "runner encoder failed with errno $?"; }
	...
	runner "cd '${WORKDIR}' && ${cmd_dec}" || { echo "runner decoder failed with errno $?"; true; }
