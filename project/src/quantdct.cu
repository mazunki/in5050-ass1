    ...
    for(int v = 0; v < MACROBLOCK_SIZE; v++)
    {
        for(int u = 0; u < MACROBLOCK_SIZE; u++)
        {
            ...
            for(int y = 0; y < MACROBLOCK_SIZE; y++)
            {
                ...
                {
                    dct += in[y*MACROBLOCK_SIZE+x] * dctlookup[x][u] * dctlookup[y][v];
                }
            }
    ...
    for(int v = 0; v < MACROBLOCK_SIZE; v++)
    {
        for(int u = 0; u < MACROBLOCK_SIZE; u++)
        {
            ...
            for(int y = 0; y < MACROBLOCK_SIZE; y++)
            {
                for(int x = 0; x < MACROBLOCK_SIZE; x++)
                {
                    dct += in[y*MACROBLOCK_SIZE+x] * dctlookup[u][x] * dctlookup[v][y];
                }
            }
    ...
    uint8_t *quant_tbl)
{
  ...
  for( int i = 0; i < MACROBLOCK_SIZE*MACROBLOCK_SIZE; i++ ) { mb[i] = in_data[i]; }

  dct_2d(mb, mb2);
  scale_block(mb2, mb);
  quantize_block(mb, mb2, quant_tbl);

  for( int i = 0; i < MACROBLOCK_SIZE*MACROBLOCK_SIZE; i++ ) { out_data[i] = mb2[i]; }
}
    ...
    int16_t *out_data, uint8_t *quantization)
{
  ...
  for(x = 0; x < w; x += MACROBLOCK_SIZE)
  {
    ...
    for (i = 0; i < MACROBLOCK_SIZE; ++i)
    {
      for (j = 0; j < MACROBLOCK_SIZE; ++j)
      {
        block[i*MACROBLOCK_SIZE+j] = ((int16_t)in_data[i*w+j+x] - prediction[i*w+j+x]);
      }
    }
